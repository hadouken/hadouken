...
desc "MSI"
task :msi => :output do
    system "tools/wix-3.6rc/candle.exe -ext WixFirewallExtension -ext WixUtilExtension -dPlatform=#{BUILD_PLATFORM} -dBuildVersion=#{BUILD_VERSION} -dBinDir=build/#{BUILD_PLATFORM}/hdkn-#{BUILD_VERSION}-#{BUILD_PLATFORM} -out src/Installer/ src/Installer/Hadouken.wxs src/Installer/UI/WebUIConfigDialog.wxs src/Installer/UI/WebUIConfigInvalidDialog.wxs src/Installer/UI/WindowsServiceConfigDialog.wxs src/Installer/UI/WindowsServiceConfigInvalidDialog.wxs src/Installer/Components/Config.wxs src/Installer/Components/Core.wxs src/Installer/Components/Firewall.wxs  src/Installer/Components/Lib.wxs  src/Installer/Components/Service.wxs"
    system "tools/wix-3.6rc/light.exe -ext WixUIExtension -ext WixFirewallExtension -ext WixUtilExtension -sval -pdbout src/Installer/Hadouken.wixpdb -out build/#{BUILD_PLATFORM}/hdkn-#{BUILD_VERSION}-#{BUILD_PLATFORM}.msi src/Installer/Hadouken.wixobj src/Installer/WebUIConfigDialog.wixobj src/Installer/WebUIConfigInvalidDialog.wixobj src/Installer/WindowsServiceConfigDialog.wixobj src/Installer/WindowsServiceConfigInvalidDialog.wixobj src/Installer/Config.wixobj src/Installer/Core.wixobj src/Installer/Firewall.wixobj  src/Installer/Lib.wixobj  src/Installer/Service.wixobj"
end