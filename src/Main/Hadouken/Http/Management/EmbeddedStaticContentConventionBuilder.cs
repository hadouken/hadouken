...
namespace Hadouken.Http.Management
{
    public class EmbeddedStaticContentConventionBuilder
    {
        ...
        public static Func<NancyContext, string, Response> AddDirectory(string requestedPath, Assembly assembly, string contentPath = null, string namespaceRoot = null, params string[] allowedExtensions)
        {
            ...
            if (namespaceRoot != null && !namespaceRoot.EndsWith("."))
            {
                namespaceRoot = string.Concat(namespaceRoot, ".");
            }
            ...
            return (ctx, root) =>
            {
                ...
                var responseFactory =
                    ResponseFactoryCache.GetOrAdd(path, BuildContentDelegate(ctx, requestedPath, contentPath, assembly, namespaceRoot, allowedExtensions));

                return responseFactory.Invoke();
            };
        }

        private static Func<string, Func<Response>> BuildContentDelegate(NancyContext context, string requestedPath, string contentPath, Assembly assembly, string namespaceRoot, string[] allowedExtensions)
        {
            return requestPath =>
            {
                ...
                var resourceName =
                    Path.GetDirectoryName(namespaceRoot + fileName.Substring(2)).Replace('\\', '.').Replace('-', '_');
