...
namespace Hadouken.Core.Services {
    public sealed class NotificationService : IJsonRpcService {
        ...
        public IEnumerable<Notifier> GetNotifiers() {
            ...
            return (from n in notifiers
                ...
                select new Notifier {
                    ...
                    RegisteredTypes = this._notifierRepository.GetTypesForNotifier(attr.ExtensionId)
                });
        }

        [JsonRpcMethod("notifiers.getConfiguration")]
        public NotifierConfiguration GetNotifierConfiguration(string notifierId) {
            var notifier = this._notifierEngine.GetAll().FirstOrDefault(n => n.GetId() == notifierId);
            if (notifier == null) {
                return null;
            }
        ...
        public void TestNotifier(string notifierId) {
            var notifier = (from n in this._notifierEngine.GetAll()
                where n.GetId() == notifierId
                select n).FirstOrDefault();

            if (notifier == null) {
                this._logger.Warn("Unknown notifier: {NotifierId}.", notifierId);
                return;
            }

            if (!notifier.CanNotify()) {
                this._logger.Warn("Notifier is not configured: {NotifierId}.", notifierId);
                return;
            }
