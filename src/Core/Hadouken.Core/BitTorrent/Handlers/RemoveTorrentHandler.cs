...
namespace Hadouken.Core.BitTorrent.Handlers {
    internal class RemoveTorrentHandler : IMessageHandler<RemoveTorrentMessage> {
        ...
        public void Handle(RemoveTorrentMessage message) {
            using (var handle = this._session.FindTorrent(message.InfoHash)) {
                if (handle == null) {
                    return;
                }
                this._session.RemoveTorrent(handle, message.RemoveData);
                this._torrentInfoRepository.Remove(message.InfoHash);
            }
        }
    }
}