...
namespace Hadouken.Common.IO {
    ...
    public sealed class DirectoryPath : Path {
        ...
        public FilePath GetFilePath(FilePath path) {
            var combinedPath = System.IO.Path.Combine(this.FullPath, path.GetFilename().FullPath);
        ...
        public DirectoryPath MakeAbsolute(DirectoryPath path) {
            ...
            return this.IsRelative
                ? path.Combine(this)
                : new DirectoryPath(this.FullPath);
        }
        ...
        public DirectoryPath MakeAbsolute(IEnvironment environment) {
            ...
            return this.IsRelative
                ? environment.GetApplicationRoot().Combine(this)
                : new DirectoryPath(this.FullPath);
        }
