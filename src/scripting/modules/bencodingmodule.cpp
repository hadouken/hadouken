...
duk_ret_t BEncodingModule::initialize(duk_context* ctx)
{
...
duk_ret_t BEncodingModule::decode(duk_context* ctx)
{
    duk_size_t size;
    ...
    BitTorrent::LazyEntryWrapper::initialize(ctx, entry);
    return 1;
}

duk_ret_t BEncodingModule::encode(duk_context* ctx)
{
    libtorrent::entry* entry = Common::getPointer<libtorrent::entry>(ctx, 0);
